...
- [ ] **Task 6.1.2:** Implementar a query no banco de dados na rota `GET /history` para recuperar apenas as conferências finalizadas (`headline IS NOT NULL AND chronicle IS NOT NULL`) associadas ao `user_id` da sessão, ordenadas por `created_at DESC`, e passá-las ao template.
